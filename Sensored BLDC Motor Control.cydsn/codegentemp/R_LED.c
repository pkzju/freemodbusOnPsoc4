...
#define SetP4PinDriveMode(shift, mode)  \
    do { \
        R_LED_PC =   (R_LED_PC & \
                                (uint32)(~(uint32)(R_LED_DRIVE_MODE_IND_MASK << (R_LED_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << (R_LED_DRIVE_MODE_BITS * (shift))); \
    } while (0)
...
{
    uint8 drVal = (uint8)(R_LED_DR & (uint8)(~R_LED_MASK));
    drVal = (drVal | ((uint8)(value << R_LED_SHIFT) & R_LED_MASK));
    R_LED_DR = (uint32)drVal;
}
...
{
	SetP4PinDriveMode(R_LED__0__SHIFT, mode);
}
...
{
    return (uint8)((R_LED_PS & R_LED_MASK) >> R_LED_SHIFT);
}
...
{
    return (uint8)((R_LED_DR & R_LED_MASK) >> R_LED_SHIFT);
}
...
/* If Interrupts Are Enabled for this Pins component */ 
    ...
    *******************************************************************************/
    ...
    {
		uint8 maskedStatus = (uint8)(R_LED_INTSTAT & R_LED_MASK);
		R_LED_INTSTAT = maskedStatus;
        return maskedStatus >> R_LED_SHIFT;
    }
