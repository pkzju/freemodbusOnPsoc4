    ...
    *******************************************************************************/
    void UART_2_UartInit(const UART_2_UART_INIT_STRUCT *config)
    {
        ...
        else
        {
            ...
            UART_2_rxBufferSize  = (uint8) config->rxBufferSize;

            /* Set TX direction internal variables */
            UART_2_txBuffer      =         config->txBuffer;
            UART_2_txDataBits    = (uint8) config->dataBits;
            UART_2_txBufferSize  = (uint8) config->txBufferSize;
...
void UART_2_UartPostEnable(void)
{
    ...
    if (UART_2_CHECK_TX_SDA_MISO_PIN_USED)
    {
        ...
        UART_2_SET_HSIOM_SEL(UART_2_TX_SDA_MISO_HSIOM_REG, UART_2_TX_SDA_MISO_HSIOM_MASK,
                                       UART_2_TX_SDA_MISO_HSIOM_POS, UART_2_HSIOM_UART_SEL);
    }
#endif /* (UART_2_TX_SDA_MISO_PIN_PIN) */

#if (UART_2_SS0_PIN)
    if (UART_2_CHECK_SS0_PIN_USED)
    {
        /* Set SCB UART to drive the output pin */
        UART_2_SET_HSIOM_SEL(UART_2_SS0_HSIOM_REG, UART_2_SS0_HSIOM_MASK,
                                       UART_2_SS0_HSIOM_POS, UART_2_HSIOM_UART_SEL);
    }
...
void UART_2_UartStop(void)
{
        ...
        if (UART_2_CHECK_TX_SDA_MISO_PIN_USED)
        {
            ...
            UART_2_SET_HSIOM_SEL(UART_2_TX_SDA_MISO_HSIOM_REG, UART_2_TX_SDA_MISO_HSIOM_MASK,
                                           UART_2_TX_SDA_MISO_HSIOM_POS, UART_2_HSIOM_GPIO_SEL);
        }
    #endif /* (UART_2_TX_SDA_MISO_PIN_PIN) */
        ...
        if (UART_2_CHECK_SS0_PIN_USED)
        {
            ...
            UART_2_spi_ss0_Write(UART_2_GET_UART_RTS_INACTIVE);

            /* Set GPIO to drive output pin */
            UART_2_SET_HSIOM_SEL(UART_2_SS0_HSIOM_REG, UART_2_SS0_HSIOM_MASK,
                                           UART_2_SS0_HSIOM_POS, UART_2_HSIOM_GPIO_SEL);
        }
    #endif /* (UART_2_SS0_PIN) */
        ...
        /* Set GPIO to drive output pin */
        UART_2_SET_HSIOM_SEL(UART_2_TX_HSIOM_REG, UART_2_TX_HSIOM_MASK,
                                       UART_2_TX_HSIOM_POS, UART_2_HSIOM_GPIO_SEL);
    #endif /* (UART_2_UART_TX_PIN) */
        ...
        /* Set GPIO to drive output pin */
        UART_2_SET_HSIOM_SEL(UART_2_RTS_HSIOM_REG, UART_2_RTS_HSIOM_MASK,
                                       UART_2_RTS_HSIOM_POS, UART_2_HSIOM_GPIO_SEL);
    #endif /* (UART_2_UART_RTS_PIN) */
...
void UART_2_UartSetRxAddressMask(uint32 addressMask)
{
    ...
    *******************************************************************************/
    CY_ISR(UART_2_UART_WAKEUP_ISR)
    {
        ...
        * only disabled by UART_2_UartRestoreConfig() call.
        */
    #if(UART_2_SCB_MODE_UNCONFIG_CONST_CFG)
        #if(UART_2_MOSI_SCL_RX_WAKE_PIN)
            (void) UART_2_uart_rx_wake_i2c_scl_spi_mosi_ClearInterrupt();
